    ...
    namespace detail {
        ...
        struct SignalImpl : boost::noncopyable {
            ...
            void clean() {
                ...
                copyOnWrite();
                SlotList &list(*slots_);  //  list  插槽s
                typename SlotList::iterator it(list.begin());
                while (it != list.end()) {
                    if (it->expired()) {  //如果该方法 过期 就删除
                        it = list.erase(it);
                    } else {
                        ++it;
                    }
        ...
        void call(ARGS &&... args) {
            ...
            for (typename SignalImpl::SlotList::const_iterator it = s.begin(); it != s.end(); ++it) {
                ...
                if (slotImpl) {
                    ...
                    if (slotImpl->tied_) {  //如果和某个对象绑定  则对象存在才可以调用该函数
...
int main() {
    ...
    using namespace std;
    Signal<void(int, int)> ss, s2;
    auto res = [](int a, int b) -> void { cout << a << "  " << b<<endl; };

    auto k = ss.connect(res);
    auto w = s2.connect(res);
    ss.call(1, 2);
    s2.call(9, 67);
}