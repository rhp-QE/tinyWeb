...
namespace QE {
    namespace base {
        ...
        Scheduler::Scheduler() :
                coroutine_num_(0), cap_(DEFAULT_COROUTINE), running_(-1), co_(vector<unique_ptr<Coroutine>>(0)) {}
 ...
 */
        ptrdiff_t
        Scheduler::add(CoroutineFunc func) {
            Coroutine *co = new Coroutine(this, func);
            this->co_.push_back(std::unique_ptr<Coroutine>(co));
        ...
        Scheduler::resume(int id) {
            ...
            switch (nowStatus) {
                case Coroutine::status::COROUTINE_READY:
                    ...
                    ptr = (uintptr_t) this;
                    makecontext(&nowContext, (void (*)(void)) mainfunc, 2, (uint32_t) ptr,
                                (uint32_t) (ptr >> 32));    //为协程设置入口函数
 ...
 */
        Coroutine::Coroutine(Scheduler *s, CoroutineFunc func) :
                func_(func), scheduler_(s), cap_(0), size_(0), status_(status::COROUTINE_READY), stack_(nullptr) {
            ::memset(&ucontext_, 0, sizeof(ucontext_));
        }
 ...
 */
        void Coroutine::saveStack(char *top) {
            ...
            if (cap_ < top - &dummy) {
                stack_.reset();
                cap_ = top - &dummy;
                stack_ =std::move(std::shared_ptr<char>(new char[cap_],[](char *ptr){delete []ptr;}));

            }
            size_ = top - &dummy;

            memcpy(stack_.get(), &dummy, size_);
        }
