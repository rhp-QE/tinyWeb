...
namespace base {
    ...
    Scheduler::Scheduler()
        : coroutine_num_(0)
        , cap_(DEFAULT_COROUTINE)
        , running_(-1)
        , co_(vector<unique_ptr<Coroutine>>(0))
    {
    }
    ...
    void Coroutine::saveStack(char* top)
    {
        ...
        if (cap_ < top - &dummy) {
            stack_.reset();
            cap_ = top - &dummy;
            stack_ = std::shared_ptr<char>(new char[cap_], [](char* ptr) { delete[] ptr; });
        }
        size_ = top - &dummy;
     ...
     * 运行协程实例
     */
    void inline Coroutine::runFunc(Scheduler* s)
    {
        func_();
    }
}
